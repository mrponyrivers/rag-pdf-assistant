...
Local-only PDF Q&A assistant built with Streamlit.  
Upload a PDF → chunk text → embed with Sentence Transformers 
→ retrieve top sources with similarity scores → generate an 
offline “local synthesis” answer + page citations.

## Project highlights
- **Cached indexing** by PDF hash + chunk settings (fast 
reloads)
- **Retrieval transparency** with similarity scores + page 
citations
- **Exports**: `answers.md`, `answers.csv`, and `sources.csv`
